...
@section('content')
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title fw-semibold mb-4">Form Edit Pesanan</h5>
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('pesanan.update', $pesanan->id) }}" method="POST">
                                ...
                                <select class="form-select form-select-2 @error('pelanggan') is-invalid @enderror"
                                    ...
                                    @foreach ($pelanggan as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('pelanggan', $pesanan->pelanggan_id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->nama }}</option>
                                    @endforeach
                                </select>
                                @error('pelanggan')
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select form-select-2 @error('paket') is-invalid @enderror"
                                    ...
                                    @foreach ($paket as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('paket', $pesanan->paket_id) == $item->id ? 'selected' : '' }}
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('berat') is-invalid @enderror"
                                    id="berat" name="berat" value="{{ old('berat', $pesanan->berat) }}"
                                    placeholder="Berat (Kg)" oninput="hitungTotalHarga()">
                                @error('berat')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('total_harga') is-invalid @enderror"
                                    ...
                                    value="{{ old('total_harga', $pesanan->total_harga) }}" placeholder="Total Harga"
                                    readonly>
                                @error('total_harga')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select form-select-2 @error('status') is-invalid @enderror"
                                    id="status" name="status">
                                    <option value="">Pilih Pelanggan</option>
                                    @foreach ($status as $item)
                                        <option value="{{ $item }}"
                                            {{ old('status', $pesanan->status) == $item ? 'selected' : '' }}>
                                            {{ $item }}</option>
                                    @endforeach
                                </select>
                                @error('status')
